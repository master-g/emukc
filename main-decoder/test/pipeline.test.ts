...
test("decodes the current cached main.js", async () => {
  ...
  expect(result.summary.scriptVersion).toBe("6.2.7.0");
  ...
  expect(result.battleKnowledge.protocolFields.some(field => field.moduleId === "83034" && field.field === "api_stage_flag")).toBe(true);
  expect(result.battleKnowledge.protocolFields.some(field => field.moduleId === "83034" && field.field === "api_kouku")).toBe(true);
  expect(result.battleKnowledge.resourceRules.some(rule => rule.moduleId === "37638" && rule.action === "getShip" && rule.targetType === "banner")).toBe(true);
  expect(result.battleKnowledge.resourceRules.some(rule => rule.moduleId === "58441" && rule.action === "getShip" && rule.targetType === "full")).toBe(true);
  expect(result.battleKnowledge.resourceRules.some(rule => rule.moduleId === "58441" && rule.action === "getSlotitem" && rule.targetType === "item_up")).toBe(true);
  ...
  expect(phaseHougeki?.source).toContain("_0x502c51.start(function() {");
  ...
  expect(shipChoiceView?.source).toContain("function ShipChoiceView(_0x31d6e6, japanese, _0x30d2f1, onFilter) {");
  expect(shipChoiceView?.source).toContain("var self = baseCtor.call(this) || this;");
  expect(shipChoiceView?.source).toContain("self.FILTER_TAB_NUM = 0x8;");
  expect(shipChoiceView?.source).toContain("self.ITEM_NUM = 0xa;");
  expect(shipChoiceView?.source).toContain("self.onFilter = onFilter");
  expect(shipChoiceView?.source).toContain("if (null == _0x31d6e6 || _0x31d6e6.length != self.FILTER_TAB_NUM) {");
  expect(shipChoiceView?.source).toContain("self._japanese = japanese");
  expect(shipChoiceView?.source).toContain("self.onFilter(_0x4c40ea, self._eventFilter.filter_status);");
  expect(shipChoiceView?.source).toContain("self.updateSelectAll(_0x20fb13);");
