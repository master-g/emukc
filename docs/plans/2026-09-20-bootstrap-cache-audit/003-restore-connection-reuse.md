...
代码改动在 `96f7689` 就已落地，缺的是步骤 1 / 步骤 4 的对照数字。本次用两个
release 二进制（一个把 `pool_max_idle_per_host` 改回 `0` 并去掉
`skip_header_check(true)`，另一个是当前代码）交替跑同一份清单，各自用一个全新的
空 `cache_root`，`--concurrent 16`，经同一个 SOCKS5 代理。

样本选择很重要。一开始按计划正文用 `head -200`，取到的 200 条**全是**
`kcs/sound/*.mp3`——那是带宽受限的场景，改前 29.28s、改后 26.17s，只差 10.6%，
容易得出「改动几乎无效」的错误结论。全量 73,050 条清单的真实构成是
49% `.mp3` + 49% `.png`（`kcs2/resources` 37,177 条、`kcs/sound` 35,302 条），
所以改用固定种子的随机 200 条重测：

| 轮次 | 改前（pool 0 + HEAD） | 改后（pool 16 + skip HEAD） |
|------|----------------------|----------------------------|
| 1 | 30.70s | 18.13s |
| 2 | 23.35s | 13.72s |
| 均值 | 27.03s（7.4 files/s） | 15.93s（12.6 files/s） |

三次对照（含 mp3-only 那次）方向一致，改后都更快，失败数三次都是 0。
随机样本上提速约 41%，吞吐 7.4 → 12.6 files/s。

CPU 时间的变化更直接地印证了机制：mp3-only 那轮 user+sys 从 1.32s 降到 0.66s，
几乎减半——每文件两次握手变成一次，且连接可复用。wall clock 的收益取决于样本里
小文件的比例，全是大 mp3 时被带宽掩盖，混合样本上才显出来。

一个踩过的坑记在这里：`Kache::builder().build()` 要求 `cache_root` **已经存在**，
`rm -rf` 之后不 `mkdir -p` 就跑，进程会以 exit 1 静默失败（错误只进日志文件，
stdout 被 populate 的进度条模式抑制）。另外配置里的相对路径是相对**配置文件所在
目录**解析的，把 `emukc.config.toml` 复制到别处当临时配置时，`workspace_root`
必须写成绝对路径，否则 codex 找不到。
