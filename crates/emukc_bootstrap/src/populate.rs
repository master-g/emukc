...
use crate::progress::{
	...
	new_spinner_on_mp, new_stats_bar, new_stats_bar_on_mp, populate_style,
...
async fn run_pass(
	...
	let failures: Arc<std::sync::Mutex<Vec<FailedItem>>> = Arc::new(std::sync::Mutex::new(Vec::new()));
	...
	loop {
		while tasks.len() < q {
			...
			let spinner = mp.as_ref().as_ref().map(|mp| {
				new_spinner_on_mp(&item_path, mp)
			});
...
pub async fn populate(
	...
	let aggregate_pb = match mp.as_ref() {
		Some(mp_ref) => {
			let pb = new_progress_bar_on_mp(total_files as u64, "Populating cache", populate_style(), mp_ref);
			Some(Arc::new(pb))
		}
		None => new_progress_bar(total_files as u64, "Populating cache", populate_style()).map(Arc::new),
	};
	...
	.await;
	debug_assert_eq!(active_count.load(Ordering::Relaxed), 0, "active_count must be 0 after run_pass");
	...
	let retry_items: Vec<(String, Option<String>)> = pass1_failures
		.into_iter()
	...
	if failed_count > 0 {
		...
		return Err(KacheError::InvalidFile(format!(
			"{failed_count} items failed after retry"
		)));
	}
