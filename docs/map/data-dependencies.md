...
运行时敌方遭遇（`sortie/enemy_ship.rs` 的 `build_enemy_encounter`）取所选 pattern 自带的陣形，
所以「随机选一个 pattern」就等于「随机选一个该节点可能的陣形」；pattern 没有陣形时取该格列表首项，
都没有时取単縦陣。2026-09-23 之前一律取格子列表首项，这句并不成立。8 个非战斗节点
