...
Known broken tests (pre-existing, not introduced by 004; surfaced at U7 verification 2026-06-22):

- `crates/emukc_gameplay/tests/sortie_battle.rs` — 3 stable-failing tests, confirmed failing at pre-004 commit `7a76553`:
  - `sortie_battle_response_passes_battle_rule_validation` (line 706)
  - `sortie_battle_result_advances_boss_quest_on_real_boss_node` (line 481)
  - `repo_wikiwiki_asset_supports_real_map_boss_progression` (line 514)
  - Root cause: battle response flag/payload mismatch (`api_hourai_flag[i]` vs presence of `api_hougeki{i+1}`). Not RNG-injection related. Likely belongs to 002 (battle-attack-system) scope. None are `#[ignore]`d. Full suite: 12 passed / 3 failed in that file.
