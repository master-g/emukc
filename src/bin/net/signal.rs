		...
		tokio::select! {
			...
			_ = async {
				if let Ok(signal) = listen().await {
					warn!(target: super::LOG_TAG, "{} received during graceful shutdown. Terminate immediately...", signal);
				} else {
					error!(target: super::LOG_TAG, "Failed to listen to shutdown signal. Terminate immediately...");
				}
